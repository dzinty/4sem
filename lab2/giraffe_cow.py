...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        # Тут может быть скорость
        
        self.velocity1 = np.array([np.zeros(int(len(nodes_coords) / 3)),-np.heaviside(-self.nodes[2,:]+100.,1)*np.heaviside(self.nodes[0,:]-124.9,1)*np.power((100.-self.nodes[2, :])/1.,1), np.zeros(int(len(nodes_coords) / 3))])
        self.velocity2 = np.array([np.zeros(int(len(nodes_coords) / 3)),-np.heaviside(-self.nodes[2,:]+100.,1)*np.heaviside(-self.nodes[0,:]+124.9,1)*np.power((100.-self.nodes[2, :])/1.,1), np.zeros(int(len(nodes_coords) / 3))])
    ...
    def snapshot(self, snap_number):
        ...
        for i in range(0, len(self.nodes[0])):
            ...
            vel.InsertNextTuple((self.velocity1[0,i], self.velocity1[1,i], self.velocity1[2,i]))
        ...
        writer.SetFileName("cow_h3d-step-" + str(snap_number) + ".vtu")
        writer.Write()
