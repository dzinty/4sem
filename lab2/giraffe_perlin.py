...
class CalcMesh:
    ...
    def __init__(self, nodes_coords, tetrs_points):
        ...
        #Сгенерим 4хмерный шум Перлина
        self.res = 40
        self.frameres = 5
        self.pnf = PerlinNoiseFactory(4, octaves=1, tile=())
        self.smth = []
        for node in self.nodes.T:
            self.smth.append((self.pnf(node[0]/self.res, node[1]/self.res,node[2]/self.res, 0) + 1) / 2 * 255 + 0.5)
        self.smth = np.array(self.smth)
    ...
    def move(self, tau, t):
        ...
        self.smth = []
        for node in self.nodes.T:
            self.smth.append((self.pnf(node[0]/self.res, node[1]/self.res,node[2]/self.res, t/self.frameres) + 1) / 2 * 255 + 0.5)
        self.smth = np.array(self.smth)
