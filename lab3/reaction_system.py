...
# Read mesh from file
mesh = Mesh('ns_for_rs/cross.xml.gz')
...
# Define source terms
f_1 = Expression('pow(x[0]-1.0,2)+pow(x[1]-1.5,2)<0.05*0.05 ? 0.1 : 0',
                 degree=1)
f_2 = Expression('pow(x[0]-2.0,2)+pow(x[1]-1.5,2)<0.05*0.05 ? 0.1 : 0',
                 degree=1)
f_3 = Constant(0)
...
timeseries_w = TimeSeries('ns_for_rs/velocity_series')
